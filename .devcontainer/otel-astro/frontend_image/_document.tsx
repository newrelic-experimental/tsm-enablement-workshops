...
export default class MyDocument extends Document<{ envString: string, file: string }> {
  static async getInitialProps(ctx: DocumentContext) {
    const sheet = new ServerStyleSheet();
    const originalRenderPage = ctx.renderPage;
    const file = await fs.readFile(process.cwd() + '/browseragent.js', 'utf8');
    ...
    return (
      <Html>
        <Head>
          ...
          <script type="text/javascript" dangerouslySetInnerHTML={{ __html: this.props.file }}></script>
        </Head>
        <body>
          <Main />
