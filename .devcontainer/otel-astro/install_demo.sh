...
create_cluster () {
   echo "Creating your cluster, please wait..."
...
deploy_demo () {
   ...
   while true; do
      echo "Enter your ingest license key: "
      read -t 60 licenseKey
      if [ -z $licenseKey ]; then
         echo "License Key can't be empty"
         continue
      fi
      break
   done
   ...
   while true; do
      echo "Enter your New Relic datacenter: [EU/US]"
      read -t 60 datacenter
      if [ -z $datacenter ]; then
         echo "You need to choose a datacenter"
         continue
      fi
      break
   done
   ...
   echo "Demo installed, waiting for all pods to be ready, this can take while, please wait..."
   sleep 3
   ...
   clear
   echo "Access frontend via "https://$CODESPACE_NAME-3000.app.github.dev/""
}
